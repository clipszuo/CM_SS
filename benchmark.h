...
public:
    ...
    void TopKError(double alpha){
        AbsVector CMs = {
                new CM_SS<DATA_TYPE, COUNT_TYPE, 8>(400000),
	    };
        std::cout << "TopKError" << std::endl;
        BenchInsert(CMs);

        for(auto CM : CMs){
            FPICheckError(CM, alpha * TOTAL);
            delete CM;
        }
    }
    ...
    void Thp(){
        for(uint32_t i = 0;i < 5;++i){
	        ...
	        AbsVector FPIs = {
	                new OO_PE<DATA_TYPE, COUNT_TYPE>(3, 20000000 / 3.0 / (BITSIZE + sizeof(COUNT_TYPE))),
                    new CM_SS<DATA_TYPE, COUNT_TYPE, 8>(200000),
		    };
